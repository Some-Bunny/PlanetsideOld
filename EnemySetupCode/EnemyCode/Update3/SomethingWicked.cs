	...
	class SomethingWickedEnemy : MonoBehaviour
	{
		...
		public static void InitDummyEnemy()
		{
			...
			if (!flag2)
			{
				...
				PlanetsideModule.Strings.Enemies.Set("#WICKEDNESS_LONGDESC", "The shade of a priest who resided in the Gungeon since before the Bullet struck, disgusted by the Lich's practices, sook to reclaim the Gungeon and restore it to its formor glory\n\n\n *Text illegible* \n\n\nNow, all that remains of him are the shrines scattered about the Gungeon, with those kneeling at it enacting the beliefs of the priest, unable to do so himself.");
		...
		private IEnumerator TPCooldown()
		{
			...
			HasTPed = true;
			float Wait = HasPickedUpRedCasing ? 0.75f : 0;
			float Divide = HasPickedUpRedCasing ? 30 : 20;
        ...
        {
			CanBeDMG = false;
			yield return new WaitForSeconds((MaxSpeed *1.5f));
			CanBeDMG = true;
			yield break;
		}
		...
		void Glow()
        {
			if (m_currentTargetPlayer && HasPickedUpRedCasing != true && CanBeDMG == true)
            {
				base.gameObject.GetComponent<tk2dBaseSprite>().sprite.renderer.material.SetFloat("_EmissivePower", 100);
			}
			else
            {
