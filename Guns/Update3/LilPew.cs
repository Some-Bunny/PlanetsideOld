	...
	public class LilPew : GunBehaviour
	{
		...
		public override void OnReloadPressed(PlayerController player, Gun bruhgun, bool bSOMETHING)
		{
			if (gun.IsReloading && this.HasReloaded)
			{
                ...
                {
					Exploder.DoRadialPush(base.gameObject.transform.PositionVector2(), 20 * gun.ClipShotsRemaining,4);
					Exploder.DoRadialKnockback(base.gameObject.transform.PositionVector2(), 20 * gun.ClipShotsRemaining, 4);
					Exploder.DoRadialMinorBreakableBreak(base.gameObject.transform.PositionVector2(), 4);
					GameObject PoofVFX = GameManager.Instance.RewardManager.D_Chest.VFX_PreSpawn;
					PoofVFX.SetActive(true);
					UnityEngine.Object.Instantiate<GameObject>(PoofVFX, gun.CurrentOwner.transform.position - new Vector3(1.5f, 1), Quaternion.identity);
					...
					if (proj != null)
					{
						...
						if (isBem != true)
						{
							...
							if (ae)
							{
								...
								FistComponent.FistReflectBullet(proj, gun.CurrentOwner.gameActor, proj.baseData.speed *= 1+ gun.ClipShotsRemaining / 5, proj.Direction.ToAngle() + 180, 1f, proj.IsBlackBullet ? 6 : 3, 0f);
							}
						}
					}
				}
